...
import (
	...
	"github.com/jaegertracing/jaeger/demo/prototest"
)
...
func TestTraceSpanIDMarshalProto(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			ref1 := demo.SpanRef{TraceID: demo.NewTraceID(2, 3), SpanID: demo.NewSpanID(11)}
...
func TestSpanIDFromBytes(t *testing.T) {
	...
	for _, data := range errTests {
		_, err := demo.SpanIDFromBytes(data)
		require.Error(t, err)
		assert.Equal(t, err.Error(), "invalid length for SpanID")
	}

	spanID, err := demo.SpanIDFromBytes([]byte{0, 0, 0, 0, 0, 0, 0, 13})
	require.NoError(t, err)
	assert.Equal(t, spanID, demo.NewSpanID(13))
}
...
func TestTraceIDFromBytes(t *testing.T) {
	...
	for _, data := range errTests {
		_, err := demo.TraceIDFromBytes(data)
		require.Error(t, err)
	...
	tests := []struct {
		data     []byte
		expected demo.TraceID
	}{
		{data: []byte{0, 0, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0, 0, 0, 0, 3}, expected: demo.NewTraceID(2, 3)},
		{data: []byte{0, 0, 0, 0, 0, 0, 0, 2}, expected: demo.NewTraceID(0, 2)},
	}
	for _, test := range tests {
		traceID, err := demo.TraceIDFromBytes(test.data)
		require.NoError(t, err)
